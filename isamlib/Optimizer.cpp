                                    ...
                                    double& gain_ratio_denominator)
{
  ...
  {
    gain_ratio_denominator =
        delta * (2 * alpha * h_sd_norm - delta) / (2 * alpha);
  ...
  else
  {
    ...
    gain_ratio_denominator =
        .5 * alpha * (1 - beta) * (1 - beta) * h_sd_norm * h_sd_norm +
        beta * (2 - beta) * current_SSE_at_linpoint;
...
void Optimizer::relinearize(const Properties& prop)
{
  ...
  VectorXd h_gn =
      compute_gauss_newton_step(jac, &function_system._R);  // modifies _R
  ...
  if (prop.method == DOG_LEG)
  {
    ...
    // Get the value of the sum-of-squared errors at the current linearization
    // point.
    ...
    if (alpha_denominator > 0)
    {
      ...
      do
      {
        ...
        // NOTE: Here we use -h_gn because of the weird sign change in the exmap
        // functions.
        h_dl = compute_dog_leg(alpha, -gradient, -h_gn, Delta, rho_denominator);

        // Update the estimate.
        // NOTE:  Here we use -h_dl because of the weird sign change in the
        // exmap functions.
...
bool Optimizer::powells_dog_leg_update(double epsilon1, double epsilon3,
                                       SparseSystem& jacobian, VectorXd& f_x,
                                       VectorXd& grad)
{
                                             ...
                                             const Properties& prop)
{
  if (prop.method == DOG_LEG)
  {
    ...
    g_new.tail(W.num_cols() - gradient.size()) =
        increment.tail(W.num_cols() - gradient.size());
...
void Optimizer::update_estimate(const Properties& prop)
{
  ...
  {  // method == DOG_LEG
    ...
    if (alpha_denominator > 0)
    {
      ...
      VectorXd h_dl =
          compute_dog_leg(alpha, -gradient, -h_gn, Delta, rho_denominator);
      function_system.apply_exmap(-h_dl);
...
void Optimizer::gauss_newton(const Properties& prop, int* num_iterations)
{
  ...
  // Compute Gauss-Newton step h_{gn} to get to the next estimated optimizing
  // point.
...
void Optimizer::levenberg_marquardt(const Properties& prop, int* num_iterations)
{
  ...
  // Record the absolute sum-of-squares error (i.e., objective function value)
  // here.
  double error = r.squaredNorm();
  ...
  VectorXd delta =
      compute_gauss_newton_step(jacobian, &function_system._R, lambda);
...
void Optimizer::powells_dog_leg(int* num_iterations, double delta0,
                                int max_iterations, double epsilon1,
                                double epsilon2, double epsilon3)
{
  // Batch optimization
  int num_iter = 0;
  ...
  while ((not found) && (max_iterations == 0 || num_iter < max_iterations))
  {
    ...
    else
    {
      ...
      double rho =
          (f_x.squaredNorm() - f_x_new.squaredNorm()) / (rho_denominator);
      if (rho > 0)
      {
        // accept new estimate
        cout << "accepted" << endl;
...
void Optimizer::batch_optimize(const Properties& prop, int* num_iterations)
{
  ...
  switch (prop.method)
  {
    case GAUSS_NEWTON:
      ...
      break;
    case DOG_LEG:
      powells_dog_leg(num_iterations, delta0, prop.max_iterations,
                      prop.epsilon1, prop.epsilon2,
                      prop.epsilon3);  // modifies x0,R
      break;
