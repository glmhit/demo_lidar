...
void SparseMatrix::_allocate_SparseMatrix(int num_rows, int num_cols,
                                          int max_num_rows, int max_num_cols,
                                          bool init_rows)
{
  _num_rows = num_rows;
  _num_cols = num_cols;
...
{
  _allocate_SparseMatrix(mat._num_rows, mat._num_cols, mat._num_rows,
                         mat._num_cols, false);
  for (int row = 0; row < _num_rows; row++)
  {
...
{
  requireDebug(num_rows >= 0 && num_cols >= 0, "SparseMatrix constructor: "
                                               "Index out of range");
  // we reserve space for at least 2 times what we need now, but not to be less
  // than MIN_COLS
  _allocate_SparseMatrix(num_rows, num_cols, max(MIN_NUM_ROWS, 2 * num_rows),
                         max(MIN_NUM_COLS, 2 * num_cols));
}
...
SparseMatrix::SparseMatrix(const SparseMatrix& mat, int num_rows, int num_cols,
                           int first_row, int first_col)
{
  ...
  for (int row = 0; row < num_rows; row++)
  {
    ...
    *_rows[row] =
        SparseVector(*mat._rows[row + first_row], num_rows, first_row);
  }
}
...
{
  requireDebug(
      (row >= 0) && (row < _num_rows) && (col >= 0) && (col < _num_cols), "Spar"
                                                                          "seMa"
                                                                          "trix"
                                                                          "::"
                                                                          "oper"
...
void SparseMatrix::set(int row, int col, const double val, bool grow_matrix)
{
  ...
  {
    requireDebug(row < _num_rows && col < _num_cols, "SparseMatrix::set: Index "
                                                     "out of range.");
  }
...
{
  requireDebug(row >= 0 && col >= 0 && row < _num_rows && col < _num_cols, "Spa"
                                                                           "rse"
                                                                           "Mat"
                                                                           "rix"
                                                                           "::"
...
{
  out << "%triples: (" << _num_rows << "x" << _num_cols << ", nnz:" << nnz()
      << ")" << endl;
  out.precision(12);
  for (int row = 0; row < _num_rows; row++)
  {
...
{
  requireDebug(row >= 0 && row < _num_rows, "SparseMatrix::get_row: Index out "
                                            "of range.");
  return *_rows[row];
}
...
{
  requireDebug(row >= 0 && row < _num_rows, "SparseMatrix::set_row: Index out "
                                            "of range.");
  *_rows[row] = new_row;
}
...
{
  requireDebug(num >= 1, "SparseMatrix::append_new_rows: Cannot add less than "
                         "one row.");
  int pos = _num_rows;
...
{
  requireDebug(num >= 1, "SparseMatrix::append_new_cols: Cannot add less than "
                         "one column.");
  if (_num_cols + num > _max_num_cols)
  {
    // this is actually used in OrderedSparseMatrix for the column translation
    // table
...
{
  requireDebug(num_rows > 0, "SparseMatrix::ensure_num_rows: num_rows must be "
                             "positive.");
...
{
  requireDebug(num_cols > 0, "SparseMatrix::ensure_num_cols: num_cols must be "
                             "positive.");
...
{
  requireDebug(_num_rows > 0, "SparseMatrix::remove_row called on empty "
                              "matrix.");
...
void SparseMatrix::apply_givens(int row, int col, double* c_givens,
                                double* s_givens)
{
  requireDebug(row >= 0 && row < _num_rows && col >= 0 && col < _num_cols, "Spa"
                                                                           "rse"
                                                                           "Mat"
                                                                           "rix"
                                                                           "::"
                                                                           ...
                                                                           "rix"
                                                                           ".");
  requireDebug(row > col, "SparseMatrix::apply_givens: can only zero entries "
                          "below the diagonal.");
...
{
  requireDebug(lhs.num_cols() == rhs.rows(), "SparseMatrix::operator* matrix "
...
VectorXd mul_SparseMatrixTrans_Vector(const SparseMatrix& lhs,
                                      const VectorXd& rhs)
{
  requireDebug(lhs.num_rows() == rhs.rows(), "SparseMatrix::mul_"
                                             "SparseMatrixTrans_Vector matrix "
                                             "and vector incompatible.");
