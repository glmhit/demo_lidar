...
// code to account for angle axis to euler state represention change when using
// quaternions this would evaluate to identity if exmap wasn't mapping between
// euler and angle axis something similar can be use to trasform covariance
// recovery output when using quats
...
#ifdef GLC_DEBUG
  double recreate_test =
      (B.transpose() * B - A).array().abs().matrix().lpNorm<Infinity>();
...
MatrixXd glc_get_weighted_jacobian(isam::Factor* f)
{
  ...
  int offset = 0;
  for (Terms::const_iterator it = jac.terms().begin(); it != jac.terms().end();
       it++)
  {
       ...
       it++)
  {
    ...
    for (size_t i = 0; i < f_nodes.size(); i++)
    {
      ...
      if (id_e != id_i)
      {
        ...
        if (node_list.end() ==
            find(node_list.begin(), node_list.end(), f_nodes[i]))
        {  // not already added
...
vector<Factor*> glc_intra_clique_factors(vector<Node*> clique_nodes, Node* node)
{
  ...
  for (size_t i = 0; i < clique_nodes.size(); i++)
  {
    ...
    for (list<Factor*>::const_iterator it = factors.begin();
         it != factors.end(); it++)
    {
      ...
      // nodes in these factors can be: the marg node, nodes in the clique, or
      // nodes ouside the clique we wish to return factors strictly between
      // nodes in the clique, not outside nor margnode is the marg node in this
      // factor
      ...
      for (size_t j = 0; j < f_nodes.size() && ic; j++)
      {
        ...
        if (clique_nodes.end() ==
            find(clique_nodes.begin(), clique_nodes.end(), f_nodes[j]))
          ic = false;
      }
...
MatrixXd glc_target_info(Node* node, vector<Node*>& clique_nodes,
                         vector<Factor*>& ic_factors)
{
  ...
  for (list<Factor*>::iterator it = factors.begin(); it != factors.end(); it++)
  {
    ...
    ////HACK TO THROW AWAY Z PRIORS (for comparision with compounding that also
    ///throws away z-priors)
    ...
    for (size_t i = 0; i < f_nodes.size(); i++)
    {
      ...
      for (size_t j = i; j < f_nodes.size(); j++)
      {
        ...
        L.block(koff, loff, nk->dim(), nl->dim()) +=
            dL.block(ioff, joff, ni->dim(), nj->dim());
        if (ni != nj)  // off diagonal elements
          L.block(loff, koff, nl->dim(), nk->dim()) +=
              dL.block(joff, ioff, nj->dim(), ni->dim());
...
#ifdef GLC_DEBUG
    cout << "[glc]\tAdding info from " << f->name()
  ...
  MatrixXd L_marg =
      L.topLeftCorner(n, n) -
      (L.topRightCorner(n, node->dim()) * posdef_pinv(Lbb, GLC_EPS) *
       L.bottomLeftCorner(node->dim(), n));
  // cout << "L_marg" << endl << L_marg << endl;
  return L_marg;
}

Factor* glc_factor(const MatrixXd& L, const vector<Node*>& clique_nodes,
                   GLC_Reparam* rp)
{
  int np = clique_nodes.size();
  int n = 0;
  for (int i = 0; i < np; i++)
    n += clique_nodes[i]->dim();
  ...
  Factor* f_add = NULL;
  if (rp == NULL)
  {  // H with linearization on world frame xfms
    ...
    for (int i = 0; i < np; i++)
    {
      x.segment(off, clique_nodes[i]->dim()) =
          clique_nodes[i]->vector(LINPOINT);
  ...
  {  // H with linearization on root shifted xfms
    ...
    MatrixXd H = G * F;
    double recreate_test =
        (H.transpose() * H - Lp).array().abs().matrix().lpNorm<Infinity>();
  ...
  if (f_add != NULL)
  {
    cout << "[glc]\t\tComputed GLC_Factor " << f_add->unique_id()
         << " between nodes ";
    for (int i = 0; i < np; i++)
...
vector<Factor*> glc_lift_factors(const MatrixXd& L,
                                 const vector<Node*>& clique_nodes, bool sparse,
                                 GLC_Reparam* rp)
{
  ...
  if (sparse)
  {
    ...
    for (it = clt.tree.begin(); it != clt.tree.end(); it++)
    {
      ...
      else
      {
        ...
        Hagb.block(0, adim, adim, bdim) =
            posdef_pinv(Laa) * Lab.block(0, adim, adim, bdim);
...
vector<Factor*> glc_remove_node(Slam& slam, Node* node, bool sparse,
                                GLC_Reparam* rp)
{
  // get the nodes in the elimination clique, new glc factor(s) will span these
  // nodes
  ...
  // intra-clique factors, those in the clique that are not directly connected
  // to the marg node not strictly required, initial icra paper included them
  vector<Factor*> ic_factors;
#ifdef GLC_INCLUDE_IC_FACTORS
...
#ifdef GLC_DEBUG
    cout << "[glc]\tAdded GLC Factor: " << new_glc_factors[i]->unique_id()
         << endl;
#endif
  }
