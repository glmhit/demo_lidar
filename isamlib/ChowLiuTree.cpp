...
ChowLiuTree::ChowLiuTree(const Eigen::MatrixXd& L,
...
double ChowLiuTree::_calc_mi(int ida, int idb)
{
  ...
  double ldL_agb = log(
      (L_agb + MatrixXd::Identity(L_agb.rows(), L_agb.cols())).determinant());
  double ldL_a =
      log((L_a + MatrixXd::Identity(L_a.rows(), L_a.cols())).determinant());
  double mi = 0.5 * (ldL_agb - ldL_a);
  ...
  while (edge != _edges.end())
  {
    if (groups[edge->id1] != groups[edge->id2])
    {
      ...
      for (groupIt = groups.begin(); groupIt != groups.end(); groupIt++)
        if (groupIt->second == group2)
          groupIt->second = group1;
