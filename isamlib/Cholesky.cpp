...
public:
  ...
  void factorize(const SparseSystem& Ab, VectorXd* delta = NULL,
                 double lambda = 0)
  {
    tic("Cholesky");
    ...
    // make sure factorization is in correct format (LL, simplicial, packed,
    // ordered)
    cholmod_change_factor(CHOLMOD_REAL, true, false, true, true, L_factor,
                          &Common);
    ...
    if (delta)
    {
      cholmod_dense* delta_ =
          cholmod_solve(CHOLMOD_Lt, L_factor, _rhs, &Common);
    ...
    // WARNING: L_factor becomes symbolic!! (numeric L is literally pulled out
    // for efficiency)
    _order = new int[ncol];
...
private:
  ...
  {
    if (_L)
      cholmod_free_sparse(&_L, &Common);
    if (_rhs)
      cholmod_free_dense(&_rhs, &Common);
    if (_order)
      delete[] _order;
  }

  // internal, allocates new cholmod matrix and copies transpose of SparseSystem
  // over (SparseSystem is row-based, cholmod column-based)
  cholmod_sparse* to_cholmod_transp(const SparseSystem& A)
  {
    // note: num_cols/num_rows swapped for transpose
    cholmod_sparse* T =
        cholmod_allocate_sparse(A.num_cols(), A.num_rows(), A.nnz(), true, true,
                                0, CHOLMOD_REAL, &Common);
  ...
  // internal, returns SparseSystem containing a copy of cholmod matrix
  // transposed for efficiency
