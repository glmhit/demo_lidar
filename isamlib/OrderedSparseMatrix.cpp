...
void OrderedSparseMatrix::_copy_from_OrderedSparseMatrix(
...
{
  delete[] _r_to_a;
  _r_to_a = NULL;  // recommended, as freeing a second time has unpredictable
                   // consequences
  delete[] _a_to_r;
  _a_to_r = NULL;
}

void OrderedSparseMatrix::_calc_reverse_order(int num, const int* order,
                                              int* reverse_order) const
{
  for (int i = 0; i < num; i++)
  {
...
OrderedSparseMatrix::OrderedSparseMatrix(int num_rows, int num_cols)
...
const OrderedSparseMatrix& OrderedSparseMatrix::
operator=(const OrderedSparseMatrix& mat)
{
  if (this == &mat)
    return *this;
...
void OrderedSparseMatrix::import_rows_ordered(int num_rows, int num_cols,
                                              SparseVector_p* rows, int* r_to_a)
{
  _dealloc_OrderedSparseMatrix();
...
void OrderedSparseMatrix::append_new_cols(int num)
{
  ...
  if (orig_max_num_cols != _max_num_cols)
