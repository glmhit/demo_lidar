 ...
 * @input eps Numerical epsilon to determine zero singular values (defaults to
 * std::numeric_limits::eps)
       ...
       double eps = std::numeric_limits<typename T::Scalar>::epsilon())
{
  ...
  T result = svd.matrixV() *
             T((svd.singularValues().array().abs() > tolerance)
...
/**
 * Calculate the pseudo inverse of an positive semidefinite matrix using the
              ...
              double eps = std::numeric_limits<typename T::Scalar>::epsilon())
{
  ...
  typename T::Scalar tolerance = eps * std::max(a.cols(), a.rows()) *
                                 eig.eigenvalues().array().abs().maxCoeff();

  T result = eig.eigenvectors() *
             T((eig.eigenvalues().array() > tolerance)
...
#ifdef NDEBUG
// Release mode
// remove requirements in inner loops for speed, but keep standard require
// functional
