 ...
 * @brief Provides specialized nodes and factors for depthmono vision
 * applications.
...
{  // for now, camera and robot are identical
  double _f;
  Eigen::Vector2d _pp;
  double _b;
...
public:
  ...
  Point3dh backproject(const Pose3d& pose,
                       const DepthmonoMeasurement& measure) const
  {
    ...
    if (lz < 0.)
    {
      std::cout << "Warning: DepthmonoCamera.backproject called with negative "
                   "disparity\n";
    }
...
public:
  ...
  Depthmono_Factor(Pose3d_Node* pose, Point3dh_Node* point,
                   DepthmonoCamera* camera, const DepthmonoMeasurement& measure,
                   const Noise& noise, bool relative = false)
  ...
  Depthmono_Factor(Pose3d_Node* pose, Point3d_Node* point,
                   DepthmonoCamera* camera, const DepthmonoMeasurement& measure,
                   const Noise& noise, bool relative = false)
    : FactorT<DepthmonoMeasurement>("Depthmono_Factor", 3, noise, measure)
    , _pose(pose)
  ...
  void initialize()
  {
    require(_pose->initialized(), "Depthmono_Factor requires pose to be "
                                  "initialized");
    bool initialized =
        (_point_h != NULL) ? _point_h->initialized() : _point->initialized();
    if (!initialized)
    {
      Point3dh predict;
      if (_relative)
  ...
  Eigen::VectorXd basic_error(Selector s = ESTIMATE) const
  {
    ...
    {
      // std::cout << "Warning - DepthmonoFactor.basic_error: point behind
      // camera, dropping term.\n";
      std::cout << "Warning - DepthmonoFactor.basic_error: " << this
                << " #: " << _nodes[1]->factors().size() << " "
                << _nodes[0]->factors().size() << " \npoint: " << point
                << " ptr: " << _point << " pose: " << pose << " ptr: " << _pose
