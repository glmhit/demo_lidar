...
public:
  ...
  static Eigen::Matrix3d euler_to_wRo(double yaw, double pitch, double roll)
  {
    ...
    double sss = ss_ * __s;
    Eigen::Matrix3d wRo;
    wRo << cc_, css - s_c, csc + s_s, sc_, sss + c_c, ssc - c_s, -_s_, _cs, _cc;
    return wRo;
  }

  static void wRo_to_euler(const Eigen::Matrix3d& wRo, double& yaw,
  ...
  static void quat_to_euler(Eigen::Quaterniond q, double& yaw, double& pitch,
                            double& roll)
  {
    const double q0 = q.w();
    const double q1 = q.x();
    ...
    roll = atan2(2.0 * (q0 * q1 + q2 * q3),
                 q0 * q0 - q1 * q1 - q2 * q2 +
                     q3 * q3);  // numerically more stable (thanks to Dehann for
                                // pointing this out)
...
public:
  ...
  Rot3d()
    : _quat(Eigen::Quaterniond(1, 0, 0, 0))
    , _wRo_cached(true)
    , _wRo(Eigen::Matrix<double, 3, 3>::Identity())
    , _ypr_cached(true)
    , _yaw(0.)
    , _pitch(0.)
    , _roll(0.)
  ...
  Rot3d(const Eigen::Quaterniond& quat)
    : _quat(quat), _wRo_cached(false), _ypr_cached(false)
  {
  }
  ...
  {
    out << yaw() << " " << pitch() << " " << roll() << " (quat: " << _quat.w()
        << " " << _quat.x() << " " << _quat.y() << " " << _quat.z() << ")";
  }
  ...
  {
    ensure_ypr();
    set(yaw, _pitch, _roll);
  }
  void set_pitch(double pitch)
  {
    ensure_ypr();
    set(_yaw, pitch, _roll);
  }
  void set_roll(double roll)
  {
    ensure_ypr();
    set(_yaw, _pitch, roll);
  }
  ...
  Rot3d exmap(const Eigen::VectorXd& delta) const
  {
    ...
    Eigen::Matrix3d m;
    m << 1 + (-a * a - b * b) * C, a * S - b * c * C, b * S + a * c * C,
