...
// Elementary Jacobian for one specific variable/node, potentially containing
...
public:
  ...
  const Eigen::MatrixXd& term() const
  {
    return _term;
  }
...
public:
  ...
  inline Jacobian(const double* residual, int size)
    : _dimtotal(0), _residual(size)
  {
  ...
  {
    int i = 1;
    for (Terms::const_iterator it = _terms.begin(); it != _terms.end();
         it++, i++)
    {
      out << "Term " << i << ":" << std::endl;
      out << *it;
