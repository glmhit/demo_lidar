...
public:
   ...
   * cost functions. Instead of cost_squared, use NULL, which avoids calculating
   * square roots.
   */
...
private:
  /**
   * Apply a delta vector to the linearization point and store result as new
   * estimate.
   */
  ...
  /**
   * Update the system with any newly added measurements. The measurements will
   * be appended to the existing factor matrix, and the factor is transformed
   * into triangular form again using Givens rotations. Very efficient for
   * exploration O(1), but can be more expensive otherwise >O(n).
   */
