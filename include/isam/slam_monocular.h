...
public:
                       ...
                       double distance = 5.) const
  {
    ...
    if (lz < 0.)
    {
      std::cout << "Warning: MonocularCamera.backproject called with negative "
                   "distance\n";
    }
...
public:
  ...
  Monocular_Factor(Pose3d_Node* pose, Point3dh_Node* point,
                   MonocularCamera* camera, const MonocularMeasurement& measure,
  ...
  // constructor for Euclidean geometry - WARNING: only use for points at short
  // range
  Monocular_Factor(Pose3d_Node* pose, Point3d_Node* point,
                   MonocularCamera* camera, const MonocularMeasurement& measure,
                   const isam::Noise& noise)
    : FactorT<MonocularMeasurement>("Monocular_Factor", 2, noise, measure)
    , _pose(pose)
  ...
  void initialize()
  {
    require(_pose->initialized(), "Monocular_Factor requires pose to be "
                                  "initialized");
    bool initialized =
