...
public:
  ...
  Eigen::Vector2d left_pixel() const
  {
    Eigen::Vector2d V(2);
    V << u, v;
    return V;
  ...
  Eigen::Vector2d right_pixel() const
  {
    Eigen::Vector2d V(2);
    V << u2, v;
    return V;
...
public:
  ...
  Point3dh backproject(const Pose3d& pose,
                       const StereoMeasurement& measure) const
  {
    ...
    if (lw < 0.)
    {
      std::cout << "Warning: StereoCamera.backproject called with negative "
                   "disparity\n";
    }
...
public:
  ...
  void initialize()
  {
    require(_pose->initialized(), "Stereo_Factor requires pose to be "
                                  "initialized");
    bool initialized =
  ...
  Eigen::VectorXd basic_error(Selector s = ESTIMATE) const
  {
    ...
    {
      // std::cout << "Warning - StereoFactor.basic_error: point behind camera,
      // dropping term.\n";
