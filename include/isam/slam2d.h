...
public:
   ...
   * @param measure The relative measurement from pose1 to pose2 (pose2 in
   * pose1's frame).
   * @param noise The 3x3 square root information matrix (upper triangular).
   * @param anchor1 Optional anchor node for trajectory to which pose1 belongs
   * to.
   * @param anchor2 Optional anchor node for trajectory to which pose2 belongs
   * to.
   */
    ...
    , _pose2(pose2)
  {
    ...
    if (anchor1)
    {  // offset between two relative pose graphs
      _nodes.resize(4);
      _nodes[2] = anchor1;
      _nodes[3] = anchor2;
    }
  ...
  void initialize()
  {
    ...
    require(pose1->initialized() || pose2->initialized(), "slam2d: "
                                                          "Pose2d_Pose2d_"
                                                          "Factor requires "
                                                          "pose1 or pose2 to "
                                                          "be initialized");
    ...
    if (_nodes.size() == 4)
    {
      ...
      if ((anchor1->parent() != NULL && anchor2->parent() != NULL &&
           anchor1->parent() != anchor2->parent()) ||
  ...
  {
    if (_nodes.size() == 4)
    {  // symbolic available only without anchor nodes
...
public:
   ...
   * @param measure The relative observation of the landmark in the pose's
   * frame.
   * @param noise The 2x2 square root information matrix (upper triangular).
   */
  Pose2d_Point2d_Factor(Pose2d_Node* pose, Point2d_Node* point,
                        const Point2d& measure, const Noise& noise)
    : FactorT<Point2d>("Pose2d_Point2d_Factor", 2, noise, measure)
    , _pose(pose)
    , _point(point)
  {
    _nodes.resize(2);
  ...
  void initialize()
  {
    require(_pose->initialized(), "slam2d: Pose2d_Point2d_Factor requires pose "
                                  "to be initialized");
    if (!_point->initialized())
    {
  ...
  Jacobian jacobian()
  {
    ...
    // f(x)
    double x = c * dx +
               s * dy;  // relative forward position of landmark point from pose
