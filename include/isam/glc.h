 ...
 */
std::vector<Factor*> glc_remove_node(Slam& slam, Node* node,
                                     bool sparse = false,
                                     GLC_Reparam* rp = NULL);
...
public:
   ...
   * @param rp Reparametrization object, uses to implement root-shift or user
   * defined reparameterizations to be applied before commiting to a
   * linearization point
   */
  GLC_Factor(std::vector<Node*> nodes, const Eigen::VectorXd& x,
             const Eigen::MatrixXd& G, GLC_Reparam* rp = NULL)
    : Factor("GLC_Factor", G.rows(),
             Information(Eigen::MatrixXd::Identity(G.rows(), G.rows())))
    , _x(x)
    , _G(G)
  {
  ...
  Jacobian jacobian()
  {
    ...
    int offset = 0;
    for (Terms::const_iterator it = njac.terms().begin();
         it != njac.terms().end(); it++)
    {
