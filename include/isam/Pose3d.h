 ...
 * of the vehicle: First yaw rotates around Z (changing X and Y axes to X' and
 * Y'), then pitch around the new Y' axis, and finally roll around the new X'
 * axis.
 *
 * In contrast, the returned rotation and transformation matrices are defined
 * in the opposite direction:
 * wTo transforms a point from the local (second) system to the global (first)
 * system
 *
 */
...
public:
  ...
  Pose3d(const Eigen::MatrixXd& m)
  {
    ...
    {
      require(false, "Pose3d constructor called with matrix of wrong "
                     "dimension");
    }
  }
  ...
  /**
   * Convert Pose3 to homogeneous 4x4 transformation matrix. Avoids inverting
   * wTo. The returned matrix is the world coordinate frame in the object
